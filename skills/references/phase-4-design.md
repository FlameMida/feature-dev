...
## ⚠️⚠️⚠️ 等待用户确认（CRITICAL CHECKPOINT）

**这是整个工作流中最关键的检查点 - 必须 100% 遵守**

### 关键规则

**MUST DO（必须做）**：
1. ✅ **MUST** 完成架构设计（包括 ultrathink + architect + 详细设计）
2. ✅ **MUST** 向用户展示设计方案
3. ✅ **MUST** 明确请求用户确认："**我可以开始实施了吗？**"
4. ✅ **MUST** 停止并等待用户明确回复
5. ✅ **MUST** 看到用户明确同意的词（"确认"、"可以"、"开始"、"好的"、"同意"等）
6. ✅ **MUST** 收到确认后输出确认标记和阶段完成标记
7. ✅ **MUST** 然后才能进入阶段 5（实施）

**ABSOLUTELY PROHIBITED（绝对禁止）**：
- ❌❌❌ **NEVER** 在用户未明确确认的情况下开始任何实施工作
- ❌❌❌ **NEVER** 假设用户默认同意
- ❌❌❌ **NEVER** 在展示架构设计后立即开始实施
- ❌❌❌ **NEVER** 跳过用户确认步骤
- ❌❌❌ **NEVER** 在收到用户其他类型的回复（如"看起来不错"）后直接实施（必须明确同意）
...
### 等待用户回应（MANDATORY STOP POINT）

**⚠️⚠️⚠️ THIS IS A HARD STOP - DO NOT PROCEED WITHOUT USER CONFIRMATION**
...
- ✅ 如果用户没有回应，继续等待（不要假设同意）

**绝对禁止做的**：
- ❌❌❌ **NEVER** 在用户确认前开始实施（阶段 5）
- ❌❌❌ **NEVER** 假设用户默认同意
- ❌❌❌ **NEVER** 跳过用户确认步骤
- ❌❌❌ **NEVER** 在展示设计后自动进入实施阶段
- ❌❌❌ **NEVER** 开始读取实施相关的文件
- ❌❌❌ **NEVER** 开始任何代码修改工作

### 处理用户反馈（MANDATORY PROTOCOL）

**场景 1：用户明确确认**

用户回复包含明确同意的词："确认"、"可以"、"开始"、"好的"、"同意"、"没问题"等
...
📋 本阶段产出：
- ultrathink 深度分析结果
- code-architect 的架构建议
- 完整的架构设计方案（数据库、API、服务层）
- 详细的实施步骤（X 步）

📍 下一阶段：5 - 实施

⚠️ WORKFLOW CHECKPOINT: 用户已确认，现在可以开始实施
---
```
...
⚠️ WORKFLOW CHECKPOINT: 继续阶段 4 - 架构设计
（用户要求修改，停留在阶段 4 更新设计）
---
...
然后重新等待用户确认（回到"等待用户回应"）。

**场景 3：用户没有回应或回复不明确**

用户说"看起来不错"、"挺好的"但没有明确说"可以开始"
