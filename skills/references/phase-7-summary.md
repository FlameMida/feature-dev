...
参考 [assets/output-template.md](../assets/output-template.md) 查看完整的格式化输出模板。
...
A: 不需要。总结应该面向用户和开发团队，聚焦于功能和 API，而不是代码细节。

### Q: CHANGELOG 应该如何组织？

A: 按类型分类（新增、修改、修复、移除），使用清晰的标题和简洁的描述。

### Q: OpenAPI 文档如何维护？

A: 使用代码注释自动生成（如 swag）。在代码中添加文档注释，使用工具自动生成规范。

### Q: 后续建议应该多详细？

A: 提供可操作的检查清单，但不要过于详细。聚焦于关键步骤和注意事项。
