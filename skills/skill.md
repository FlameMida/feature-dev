...
2. **并行启动 2-3 个 code-explorer agents**，每个关注不同层面：
...
**执行要点**：
1. **使用 ultrathink**（sequential-thinking）进行深度架构分析：
   - 分析需求组件和依赖关系
   - 设计数据结构（遵循 CLAUDE.md 规范）
   - 设计 API 端点（遵循 CLAUDE.md 规范）
   - 设计服务层架构
   - 识别风险和边缘情况
   - 规划详细实施步骤（至少 5-10 步，可直接执行）

2. **必须启动 code-architect agent** 获取专业架构建议：
   - 提供 ultrathink 的分析结果给 agent
   - 要求 agent 返回关键架构文件和模式建议
   - 读取 agent 识别的架构相关文件
   - 整合 agent 的建议到最终设计

3. **设计方案**（基于 ultrathink + architect 的综合分析）：
   - 简单功能：设计 1 个方案，说明理由
   - 复杂功能：设计 2-3 个备选方案（最小改动/清晰架构/实用平衡）

4. **向用户展示**：
   - 单方案：展示设计，说明理由，请求确认
   - 多方案：每个方案的简要说明 + **权衡分析** + **推荐意见及理由** + 具体差异

5. **询问用户选择**：等待用户确认使用哪个方案
